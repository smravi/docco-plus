...
var document = function(options, callback) {
    ...
    // ### run docco
    require('docco').document(options, function() {
        var indexFile = path.join(options.output, (options.index ? options.index + '.html' : 'README.md.html'));
        if (fs.existsSync(indexFile)) {
            // - copy readme file as index
            fs.copySync(
                indexFile,
...
var run = function() {
    ...
    option(
        ...
        '-i, --index [file]',
        'the file to be converted to index.html'
    ).parse(
        process.argv
    );
